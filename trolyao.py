...
def dong_co(text):
    image_1 = ImageTk.PhotoImage(Image.open("image\\fb.jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    if "không hoạt động" in text or "động cơ hoạt động yếu" in text:
        speak("Có thể do bình điện yếu hoặc đầu nối với cực của bình điện hỏng, không tiếp xúc hay bị bẩn,bugi bị hỏng. Bạn hãy kiểm tra và nạp điện hoặc thay bình điện khác. Đánh sạch và xiết chặt lại các đầu nối của bình điện. Kiểm tra bugi ")
        root.update()
        sleep_time(10)
    elif "tiêu hao nhiên liệu quá mức" in text:
        speak("Có thể do nhiên liệu kém chất lượng,bình xăng,ống dẫn và bộ chế hòa khí có hiện tượng dò xăng. Hệ thống đánh lửa quá sớm hoặc quá trễ. Chạy trong tình trạng quá tải. Cách khắc phục: Bạn hãy kiểm tra nhiên liệu và bộ phận dẫn nhiên liệu. Chỉnh lại thời điểm đánh lửa và garăngti")
        root.update()
        sleep_time(15)
    elif "bị quá nhiệt" in text:
        speak("Có thể thiết bị làm mát động cơ của bạn đã bị hỏng,chất làm mát kém chất lượng. Động cơ của xe của bạn đang làm việc quá tải. Cách khắc phục: Bạn hãy kiểm tra hệ thống làm mát động cơ :bộ điều nhiệt, ống làm mát, quạt tản nhiệt, bơm nước. Kiểm tra dầu động cơ hoặc có thể xe của bạn đang chở quá tải ")
        root.update()
        sleep_time(15)
def khoi_den():
    image_1 = ImageTk.PhotoImage(Image.open("image\\o-to-xa-khoi-den-la-vi-sao-3 (1).jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    speak("Nguyên nhân: Có thể lọc gió động cơ đã bị tắc,lọc nhiên liệu bị tắc, nhiên liệu sử dụng kém chất lượng. Kim phun nhiên liệu bị tắc hoặc Xy lanh mòn, áp suất khí kém. Cách khắc phục: Kiểm tra hệ thống lọc gió,lọc nhiên liệu và vệ sinh. Kiểm tra và vệ sinh kim phun nhiên liệu. Sử dụng đồng hồ kiểm tra áp suất buồng đốt để tìm nguyên nhân lỗi hư hỏng ở đâu và khắc phục.")
    root.update()
    sleep_time(18)
def chet_may():
    image_1 = ImageTk.PhotoImage(Image.open("image\\tt03.jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    speak("Có thể do hết nhiên liệu hoặc nhiên liệu không vào buồng đốt. Ống dẫn nhiên liệu bị dò. Hệ thống đánh lửa bị hỏng thời gian đánh lửa quá trễ hoặc quá sớm. Không khí lọt vào buồng đốt. Cách khắc phục: Bạn hãy kiểm tra hệ thống nhiên liệu và ống dẫn. Kiểm tra hệ thống làm mát, châm thêm khi gần hết. Bảo dưỡng định kì cho xe")
    root.update()
    sleep_time(19)
def toc_do_thap():
    image_1 = ImageTk.PhotoImage(Image.open("image\\thap.jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    speak("Tốc độ không tải của bạn đang để quá thấp,có thể thùng chứa còn ít nhiên liệu hoặc lõi lọc đã bị tắc. Cách khắc phục: Bạn hãy điều chỉnh lại tốc độ của động cơ ở chế độ không tải. Tiếp thêm nhiên liệu hoặc thay lõi lọc ")
    root.update()
    sleep_time(6.25)
def khoi_trang():
    image_1 = ImageTk.PhotoImage(Image.open("image\\khoitrang.jpeg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    speak("Nhiên liệu được phun vào xi lanh không được đốt cháy chính xác dẫn đến hiện tượng xe xả khói trắng. Cách khắc phục: Bạn hãy kiểm tra buồng chứa nhiên liệu xem có các tạm chất lẫn vào hoặc nước trong xăng hay không")
    root.update()
    sleep_time(6.04)
...
def youtube_search():
    ...
    if text == "":
        ...
        time.sleep(7)   
def den_khong_hoat_dong():
    import obd
    image_1 = ImageTk.PhotoImage(Image.open("image\\fb.jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
    # Tạo kết nối với động cơ ô tô qua cổng COM
    connection = obd.OBD(portstr="COM3")

    # Kiểm tra trạng thái của đèn thông báo chủ yếu
    primary_warning_cmd = obd.commands.CHECK_ENGINE
    primary_warning_response = connection.query(primary_warning_cmd)

    if primary_warning_response.value == None:
        speak("Đèn thông báo chủ yếu không hoạt động.")
    else:
        speak("Đèn thông báo chủ yếu hoạt động.")
    root.update()
    sleep_time(15)
...
def r_set():
    image_1 = ImageTk.PhotoImage(Image.open("image\\tro-ly-ao-tren-o-to-2.jpg"))
    label1 = Label(image=image_1)
    label1.image = image_1
    label1.place(x=7, y=43)
...
def ham_main():
    ...
    while True:
        ...
        elif "động cơ không hoạt động" in you or "động cơ hoạt động yếu" in you or "tiêu hao nhiên liệu quá mức" in you\
                or "quá nhiệt" in you:
            dong_co(you)
        elif "xả khói đen" in you:
            khoi_den()
        elif "khói trắng" in you:
            khoi_trang()
        elif "chết máy" in you:
            chet_may()
        elif "tốc độ thấp" in you:
            toc_do_thap()
        ...
        elif "đèn thông báo chủ yếu không hoạt động" in you:
            den_khong_hoat_dong()
...
class Example(Frame):
    ...
    def initUI(self):
        self.parent.title("Trợ lý chẩn đoán động cơ trên ô tô")
        self.style = Style()
